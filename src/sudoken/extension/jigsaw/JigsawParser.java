...
public class JigsawParser implements SectionParser {
    /**
     * Format: Positive integers in a grid the same size as puzzle, with each number specifying
     *         a cell's membership to a jigsaw piece.
     * @return 
     */
	// TODO: Ensure jigsaw pieces are contiguous.
    @Override
    public Collection<Constraint> load(String config, int width, int height) throws IOException {
        ...
        for (int i = 0; i < counts.length; i++) {
        	if (counts[i] != width) throw new IOException("Parse error: Jigsaw pieces of wrong size.");
        }
